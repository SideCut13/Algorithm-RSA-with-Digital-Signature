...
public class PanelEncryptionDecryptionRSA extends JPanel {
    private JLabel lAlgorithmRSA = new JLabel("Algorithm RSA - encryption/decryption part");
    ...
    private JButton btnEncryptFromFile =  new JButton("Encrypt with public key from file");
    ...
    private JButton btnDecryptFromFile = new JButton("Decrypt with private key from file");
    private JButton btnDecryptedTextToFile = new JButton("Save decrypted text");

    private String[] encryptedMessage;

    public PanelEncryptionDecryptionRSA() {
        ...
        gbcPanelAlgorithmRSA.insets = new Insets(10, 0, 15, 0);
        ...
        gbcPanelEncryptedText.insets = new Insets(5, 5, 5, 35);
        ...
        gbcPanelDecryptedText.insets = new Insets(5, 5, 5, 35);
        ...
        gbcPanelDecryptedText.gridy = 0;
        gbcPanelDecryptedText.insets = new Insets(5, 35, 5, 5);
        ...
        btnDecryptFromFile.setPreferredSize(new Dimension(192, 30));
        btnDecryptFromFile.setBorder(BorderFactory.createRaisedBevelBorder());
        btnDecryptFromFile.setBackground(new Color(203, 255, 181));
        btnDecryptFromFile.addActionListener(e->decryptFromFile());
        panelDencryptButton.add(btnDecryptFromFile, gbcPanelDecryptButton);

        gbcPanelDecryptButton.gridx = 1;
        gbcPanelDecryptButton.gridy = 0;
        gbcPanelDecryptButton.insets = new Insets(5, 10, 10, 5);
        btnDecryptedTextToFile.setPreferredSize(new Dimension(190, 30));
        btnDecryptedTextToFile.setBorder(BorderFactory.createRaisedBevelBorder());
        btnDecryptedTextToFile.setBackground(new Color(203, 255, 181));
        btnDecryptedTextToFile.addActionListener(e->toFile(tfDecryptedText.getText()));
        panelDencryptButton.add(btnDecryptedTextToFile, gbcPanelDecryptButton);
        //-------------------------MAIN-----------------------------------------
    ...
    //ENCRYPT MESSAGE WITH PUBLIC KEY FROM FILE
    public void encryptFromFile(){
        if(tfLoadN.getText().isEmpty() || tfLoadN.getText()==null){
            ...
            for (int i = 0; i < message.length; i++) {
                ...
                encryptedMessage[i] = String.valueOf(new BigInteger(tempStringArray[i]).modPow(e,n));
            }
            tfEncryptedText.setText(Arrays.toString(encryptedMessage).replaceAll("\\[|\\]|,", "").trim());
        }
    }
    //DECRYPT MESSAGE WITH PRIVATE KEY FROM FILE
    public void decryptFromFile(){
        ...
        if(tfEncryptedTextFromFile.getText().isEmpty() || tfEncryptedTextFromFile.getText()==null){
            ...
            String s = tfEncryptedTextFromFile.getText();
            String[] encrypted = s.split(" ");
            String[] decrypted = new String[encrypted.length];
            for (int i = 0; i < encrypted.length; i++) {
                decrypted[i] = String.valueOf(new BigInteger(encrypted[i]).modPow(d, n));
            }
